...
begin
  if _Row < FixedRows then
    begin
      ...
      if Assigned(OnGetFixedCellData) then
        begin
  ...
  else if _Col < FixedCols then
    begin
      if Assigned(OnGetRowHead) then
        begin
  ...
  else if Assigned(OnGetNonfixedCellData) then
    begin
...
begin
  ...
  try
    ...
    if not DefaultDrawing or (BgColor <> DefaultColor) then
      begin
...
procedure TCustomdzVirtualStringGrid.AdjustLineHeights(const _ResizeableCols: array of integer);
  ...
  begin
    ...
    for c := FixedCols to ColCount - 1 do
      begin
        if ArrayContains(c, _ResizeableCols) then
          begin
...
begin
  ...
  if i < FixedRows then
    i := FixedRows;
  while (i < RowCount) and (i < OldTopRow + VisibleRowCount) do
    begin
      AdjustRowHeight(i);
      Inc(i);
    end;
