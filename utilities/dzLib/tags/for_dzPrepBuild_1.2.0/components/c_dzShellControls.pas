...
{ TdzCustomShellTreeView }

  TdzCustomShellTreeView = class(TCustomTreeView, IShellCommandVerb)
  private
    FRoot,
    FOldRoot : TRoot;
...
uses ShellConsts, ShellAPI, ComObj, TypInfo, Menus, Consts, Math, StrUtils;
...
constructor TdzCustomShellTreeView.Create(AOwner: TComponent);
var
  FileInfo: TSHFileInfo;
begin
...
procedure TdzCustomShellTreeView.CreateWnd;
begin
  inherited CreateWnd;
...
procedure TdzCustomShellTreeView.CommandCompleted(Verb: String;
  Succeeded: Boolean);
var
  Fldr : TShellFolder;
begin
...
procedure TdzCustomShellTreeView.ExecuteCommand(Verb: String;
  var Handled: Boolean);
var
  szPath: array[0..MAX_PATH] of char;
begin
...
procedure TdzCustomShellTreeView.InitNode(NewNode: TTreeNode; ID: PItemIDList; ParentNode: TTreeNode);
var
  CanAdd: Boolean;
...
procedure TdzCustomShellTreeView.PopulateNode(Node: TTreeNode);
var
  ID: PItemIDList;
  EnumList: IEnumIDList;
  NewNode: TTreeNode;
...
procedure TdzCustomShellTreeView.SetObjectTypes(Value: TShellObjectTypes);
begin
  FObjectTypes := Value;
  RootChanged;
end;
...
procedure TdzCustomShellTreeView.CreateRoot;
var
  RootNode: TTreeNode;
  ErrorMsg: string;
  yToto: string;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        // twm: only use YMAG's update if it is not a CISDL
        if PathIsCSIDL(fRoot) then
          Text := GetDisplayName(DesktopShellFolder,
                                 TShellFolder(Data).AbsoluteID,
                                 SHGDN_NORMAL)
        else begin
          //update YMAG to display the last dir in place of the first dir
          yToto := AnsiReverseString(ExcludeTrailingPathDelimiter(FRoot));
          if Pos('\', yToto) > 0 then
           yToto := Copy(yToto, 1, Pos('\', yToto)-1);
          yToto := AnsiReverseString(yToto);
...
function TdzCustomShellTreeView.CanExpand(Node: TTreeNode): Boolean;
var
  Fldr: TShellFolder;
begin
  Result := True;
...
procedure TdzCustomShellTreeView.Edit(const Item: TTVItem);
var
  S: string;
  Node: TTreeNode;
begin
...
procedure TdzCustomShellTreeView.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
function TdzCustomShellTreeView.NodeFromRelativeID(ParentNode: TTreeNode; ID: PItemIDList): TTreeNode;
var
  HR: HResult;
begin
...
function TdzCustomShellTreeView.NodeFromAbsoluteID(StartNode: TTreeNode; ID: PItemIDList): TTreeNode;
var
  HR: HResult;
begin
  Result := StartNode;
...
procedure TdzCustomShellTreeView.Delete(Node: TTreeNode);
begin
...
(*
procedure TdzCustomShellTreeView.NodeDeleted(Sender: TObject; Node: TTreeNode);
begin
  if Assigned(Node.Data) then
  begin
    TShellFolder(Node.Data).Free;
...
procedure TdzCustomShellTreeView.RootChanged;
begin
...
function TdzCustomShellTreeView.FolderExists(FindID: PItemIDList; InNode: TTreeNode): TTreeNode;
var
  ALevel: Integer;
begin
  Result := nil;
...
procedure TdzCustomShellTreeView.RefreshEvent;
begin
  if Assigned(Selected) then
    Refresh(Selected);
end;

procedure TdzCustomShellTreeView.Refresh(Node: TTreeNode);
var
...
procedure TdzCustomShellTreeView.Notification(AComponent: TComponent;
...
function TdzCustomShellTreeView.CanChange(Node: TTreeNode): Boolean;
var
  Fldr: TShellFolder;
  StayFresh: boolean;
begin
...
function TdzCustomShellTreeView.GetFolder(Index: Integer): TShellFolder;
begin
  Result := TShellFolder(Items[Index].Data);
end;

function TdzCustomShellTreeView.SelectedFolder: TShellFolder;
begin
  Result := nil;
  if Selected <> nil then Result := TShellFolder(Selected.Data);
end;

function TdzCustomShellTreeView.GetPath: String;
begin
...
procedure TdzCustomShellTreeView.SetPath(const Value: string);
var
  P: PWideChar;
  NewPIDL: PItemIDList;
  Flags,
...
procedure TdzCustomShellTreeView.SetPathFromID(ID: PItemIDList);
var
  I: Integer;
  Pidls: TList;
  Temp, Node: TTreeNode;
...
procedure TdzCustomShellTreeView.SetRoot(const Value: TRoot);
begin
  if not SameText(FRoot, Value) then
  begin
    FOldRoot := FRoot;
...
procedure TdzCustomShellTreeView.GetImageIndex(Node: TTreeNode);
begin
  if Assigned(Images) then
    inherited GetImageIndex(Node);
end;

procedure TdzCustomShellTreeView.GetSelectedIndex(Node: TTreeNode);
begin
  if Assigned(Images) then
    inherited GetSelectedIndex(Node);
end;

procedure TdzCustomShellTreeView.WndProc(var Message: TMessage);
var
...
procedure TdzCustomShellTreeView.SetUseShellImages(const Value: Boolean);
var
  ImageListHandle: THandle;
begin
...
procedure TdzCustomShellTreeView.WMDestroy(var Message: TWMDestroy);
begin
  ClearItems;
  inherited;
end;

procedure TdzCustomShellTreeView.Loaded;
begin
  inherited Loaded;
  CreateRoot;
end;

procedure TdzCustomShellTreeView.DoContextPopup(MousePos: TPoint;
  var Handled: Boolean);
begin
...
procedure TdzCustomShellTreeView.SetAutoRefresh(const Value: boolean);
begin
  FAutoRefresh := Value;
