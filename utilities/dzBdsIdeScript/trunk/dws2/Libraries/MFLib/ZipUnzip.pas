...
Const   { these are stored in reverse order }
  ...
  ZIPMASTERVERSION: String = '1.52 M';
...
Type
  ...
  private
    ...
    ZipDllExec:          function( ZipRec: pZipParms ): DWord; stdcall; //v1.52m
...
begin
   ...
   begin   // We use this because DiskFree/Size don't support UNC drive names.
      ...
      begin
         LDiskFree   := {$IfDef VERD4+}LargeInt( BytesPSector )
                        {$Else}        (1.0 * BytesPSector){$EndIf}
                      * SectorsPCluster * FreeClusters;
         LSizeOfDisk := {$IfDef VERD4+}LargeInt( BytesPSector )
                        {$Else}        (1.0 * BytesPSector){$EndIf}
...
var
   ...
   NamLen:               Cardinal;
   SysLen:               {$IfDef VERD4+}DWord{$Else}Integer{$EndIf};
...
begin
   ...
   begin
      ...
      MaxLen := {$IfDef VERD4+}FFreeOnDisk{$Else}Trunc( FFreeOnDisk ){$EndIF};  // RCV150199
