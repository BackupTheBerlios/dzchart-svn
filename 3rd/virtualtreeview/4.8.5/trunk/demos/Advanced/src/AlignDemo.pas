...
begin
  ...
  if Length(ArabicStrings[0]) = 0 then begin
...
begin
  ...
  if ParentNode = nil then begin
    ...
    if not Odd(ParentNode.Index) then begin
      with Data^ do begin
...
begin
  ...
  with FArabicFont do begin
    if Screen.Fonts.IndexOf('Traditional Arabic') > -1 then begin
  ...
  with IconPopup do begin
    for I := 0 to HeaderImages.Count - 1 do begin
  ...
  with AlignTree.Header do begin
  ...
  with AlignTree.Header do begin
    ...
    if not (hoShowImages in Options) then
      Height := 24
    else if Columns[0].Layout in [blGlyphTop, blGlyphBottom] then
      Height := 64
    else
      Height := 40;
...
begin
  with AlignTree.Header do
    if ShowTextOptionBox.Checked then begin
...
begin
  with Sender as TComboBox, AlignTree.Header do begin
    for I := 0 to Columns.Count - 1 do
